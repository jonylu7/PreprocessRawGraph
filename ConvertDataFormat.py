...
def findOrderRelativeToNodeIndexByFile(orderlocation, graphFile):
    for index, nodeLocation in enumerate(graphFile["node_locations"]):
        if (abs(nodeLocation[0] - orderlocation[0]) < 1.0 and abs(nodeLocation[1] - orderlocation[1]) < 1.0 and abs(
                nodeLocation[2] - orderlocation[2]) < 1.0):
            return index
...
def preprocessROS(waypoint_graph_locations, waypoint_graph_edges, waypoint_graph_offsets, task_locations,
